...
export default function Lookup() {
  ...
  return <div><div className="page-heading"><div><p className="eyebrow">RECORDS INTELLIGENCE</p><h1>Alias & vehicle lookup</h1><p>Search fictional roleplay identities and registered vehicles—never real-world names.</p></div></div>
    <section className="panel lookup-panel"><div className="tabs"><button className={tab==='name'?'active':''} onClick={()=>{setTab('name');setResults([])}}><UserRound size={16}/> Alias</button><button className={tab==='plate'?'active':''} onClick={()=>{setTab('plate');setResults([])}}><Car size={16}/> Vehicle</button></div>
      <form className="searchbar" onSubmit={search}><Search size={19}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder={tab==='name'?'Roleplay alias or character name':'Plate number'} autoFocus/><button>Search records</button></form>
